...
class LpipsRegularization(ReferenceRegularizer):
    ...
    def __init__(self, fix_im, **kwargs):
        ...
        self.dist_model = lpips.LPIPS(net='vgg')
        if self.use_gpu:
            self.dist_model.cuda()

    def forward(self, examples, *args, **kwargs):
        xform = lambda im: im * 2.0 - 1.0
        # perceptual_loss = self.dist_model.forward_var(examples,
        #                                               self.fix_im)
        perceptual_loss = self.dist_model.forward(examples,
                                                      self.fix_im)
...
#############################################################################
#                         ciede2000 distance REGULARIZATION                 #
#############################################################################
# This is taken from the paper "Towards Large yet Imperceptible Adversarial Image
# Perturbations with Perceptual Color Distance". Code is from https://github.com/ZhengyuZhao/PerC-Adversarial
class ciede2000Regularization(ReferenceRegularizer):

    def __init__(self, fix_im, **kwargs):
        super(ciede2000Regularization, self).__init__(fix_im)
        ...
        # else:
        #     self.use_gpu = utils.use_gpu()
        self.device = torch.device("cuda" if utils.use_gpu() else "cpu")

    def forward(self, examples, *args, **kwargs):
        c = ciede2000_diff(rgb2lab_diff(examples, self.device),
                           rgb2lab_diff(self.fix_im, self.device), self.device)
        print("Color loss: ", torch.norm(c.view(1, -1), dim=1).item())
        return torch.norm(c.view(1, -1), dim=1)
