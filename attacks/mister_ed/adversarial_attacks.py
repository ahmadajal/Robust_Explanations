...
class PGD(AdversarialAttack):
    ...
    def attack(self, examples, labels, step_size=1.0/255.0,
               ...
               keep_best=True, early_stop_for=None, early_stop_value=10.0):
        ...
        for iter_no in range(max_iterations):
            ...
            if (iter_no >= min_iterations and
                ...
                break
            ####### added by me ############
            ## early stop for expl loss ####
            ################################
            if isinstance(self.loss_fxn, lf.RegularizedLoss) and early_stop_for:
                l_expl = self.loss_fxn.losses[early_stop_for]
                l_expl_value = l_expl.forward(perturbation(var_examples), var_labels,
                                             perturbation=perturbation,
                                             output_per_example=keep_best)
                if l_expl_value < early_stop_value:
                    if verbose:
                        print(" the desired {} loss achieved".format(early_stop_for))
                    break
                # to free memory
                del l_expl
                del l_expl_value
