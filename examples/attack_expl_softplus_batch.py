...
# model = convert_relu_to_softplus(model, beta=5)
####
model = model.eval()
####
...
class EXPL_Loss_mse(lf.PartialLoss):
    def __init__(self, classifier, target_expl, method, normalizer=None, smooth=False, multiply_by_input=False):
...
target_expl = get_expl(softplus_model, normalizer.forward(target_examples), args.method, desired_index=target_labels,
...
# explanation loss
expl_loss = EXPL_Loss_mse(softplus_model, target_expl, args.method, normalizer, smooth=args.smooth, multiply_by_input=args.multiply_by_input)
...
print("Final cosd", [spatial.distance.cosine(adv_expl[i].detach().cpu().flatten(),
...
org_expl = get_expl(softplus_model, normalizer.forward(examples), args.method,
