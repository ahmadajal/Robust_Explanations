...
from utils import load_pretrained_cifar10_model, get_attack_from_name, load_image, torch_to_image, get_expl, convert_relu_to_softplus
# explanation
import sys
...
model = model.eval()
# softplus model
softplus_model = convert_relu_to_softplus(model, args.beta)
...
class EXPL_Loss_mse(lf.PartialLoss):
    ...
    def forward(self, examples, labels, *args, **kwargs):
        ...
        # get target expl
        adv_expl = get_expl(self.classifier, classifier_in, self.method, desired_index=labels, smooth=self.smooth)
...
target_expl = get_expl(softplus_model, normalizer.forward(target_examples),
                            args.method, desired_index=target_label, smooth=args.smooth) # was soft-plus before!!
target_expl = target_expl.detach()
# original explanation and logits
org_expl = get_expl(softplus_model, normalizer.forward(examples),
                         args.method, desired_index=target_label, smooth=args.smooth)
...
# explanation loss
expl_loss = EXPL_Loss_mse(softplus_model, target_expl, args.method, normalizer, smooth=args.smooth)
...
adv_expl = get_expl(softplus_model, normalizer.forward(perturbation.adversarial_tensors()), method, desired_index=labels)
